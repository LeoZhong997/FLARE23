...
base_dir = "/data/result/zhongzhiqiang/nnUNet/nnUNet_outputs/predict_temp/tensorrt_test"
add_file_handler_to_logger(name="main", dir_path=base_dir + "/logs/", level="DEBUG")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("-test_name", default="trt_test_fp16", help="team docker name")
    parser.add_argument("-save_path", default=base_dir, help="save_path")
    time_interval = 0.1
    args = parser.parse_args()
    logger.info(f"we are counting: {args.test_name}")
    json_dir = join(args.save_path, args.test_name)
    csv_path = join(json_dir, args.test_name + '_Efficiency.csv')
    # jsonl = sorted(glob.glob(json_dir + "/*.json"))
    jsonl = sorted(glob.glob(json_dir + "/{}.json".format(args.test_name)))
    print("json num: ", len(jsonl))
    alldata = []
    for item in jsonl:
        csv_l = []
