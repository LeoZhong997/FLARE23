...
from nnunet.training.data_augmentation.custom_transforms import Convert3DTo2DTransform, Convert2DTo3DTransform, \
    MaskTransform, ConvertSegmentationToRegionsTransform, SpatialTransformMultiData, GaussianNoiseTransformMultiData, \
    SaveTransform, GaussianBlurTransformMultiData, BrightnessMultiplicativeTransformMultiData, \
    ContrastAugmentationTransformMultiData, SimulateLowResolutionTransformMultiData, GammaTransformMultiData
...
def get_moreDA_augmentation(dataloader_train, dataloader_val, patch_size, params=default_3D_augmentation_params,
    ...
    print("+++get_moreDA_augmentation params: ", params)
...
def get_moreDA_augmentation_multi_data(dataloader_train, dataloader_val, patch_size,
    ...
    tr_transforms.append(
        SaveTransform(save_base="/data/result/zhongzhiqiang/nnUNet/nnUNet_outputs/training_temp",
                      prefix="before")
    )
    if params.get("selected_data_channels") is not None:
        tr_transforms.append(DataChannelSelectionTransform(params.get("selected_data_channels")))
    ...
    # tr_transforms.append(SpatialTransform(
    tr_transforms.append(SpatialTransformMultiData(
        patch_size_spatial, patch_center_dist_from_border=None,
        do_elastic_deform=params.get("do_elastic"), alpha=params.get("elastic_deform_alpha"),
        sigma=params.get("elastic_deform_sigma"),
        do_rotation=params.get("do_rotation"), angle_x=params.get("rotation_x"), angle_y=params.get("rotation_y"),
    ...
    # channel gets in the way
    # tr_transforms.append(GaussianNoiseTransform(p_per_sample=0.1))
    tr_transforms.append(GaussianNoiseTransformMultiData(p_per_sample=0.1))
    # tr_transforms.append(GaussianBlurTransform((0.5, 1.), different_sigma_per_channel=True, p_per_sample=0.2,
    #                                            p_per_channel=0.5))
    tr_transforms.append(GaussianBlurTransformMultiData((0.5, 1.), different_sigma_per_channel=True, p_per_sample=0.2,
                                                        p_per_channel=0.5))
    # tr_transforms.append(BrightnessMultiplicativeTransform(multiplier_range=(0.75, 1.25), p_per_sample=0.15))
    tr_transforms.append(BrightnessMultiplicativeTransformMultiData(multiplier_range=(0.75, 1.25), p_per_sample=0.15))
    ...
    # tr_transforms.append(ContrastAugmentationTransform(p_per_sample=0.15))
    tr_transforms.append(ContrastAugmentationTransformMultiData(p_per_sample=0.15))
    # tr_transforms.append(SimulateLowResolutionTransform(zoom_range=(0.5, 1), per_channel=True,
    tr_transforms.append(SimulateLowResolutionTransformMultiData(zoom_range=(0.5, 1), per_channel=True,
                                                                 p_per_channel=0.5,
    ...
    #                    p_per_sample=0.1))  # inverted gamma
    tr_transforms.append(
        GammaTransformMultiData(params.get("gamma_range"), True, True, retain_stats=params.get("gamma_retain_stats"),
                                p_per_sample=0.1))  # inverted gamma
    ...
    if params.get("do_gamma"):
        tr_transforms.append(
            GammaTransformMultiData(params.get("gamma_range"), False, True,
