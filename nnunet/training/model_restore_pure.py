...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    if "Medium" in name:
        ...
        from nnunet.training.network_training.nnUNetTrainerV3 import nnUNetTrainerV2_FLARE_Large_Pure
        trainer = nnUNetTrainerV2_FLARE_Large_Pure(*init)
    else:
        assert "not found trainner for {}".format(name)
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best",
    ...
    if trt_mode:
        ...
        # trt_path = [join(i, "%s_trtexec_fp32.trt" % checkpoint_name) for i in folds]
