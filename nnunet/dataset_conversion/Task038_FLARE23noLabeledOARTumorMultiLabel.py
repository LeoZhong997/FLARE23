...
def process(pid, image_dir, label_dir, oar_tumor_pred_dir, pseudo_label_dir, target_imagesTr, target_labelsTr):
    ...
    if not (isfile(new_ct) and isfile(new_label)):
        ...
        if -1 not in ct_direction and -1 not in oar_tumor_pred_direction:
            ...
            if 14 in gt_unique:
                multi_tumor_npy = pseudo_label_npy + 13  # oar[1-13] to tumor multi label[14-26]
                multi_tumor_npy[gt_npy != 14] = 0        # stay tumor region
                oar_tumor_npy[gt_npy == 14] = 27     # relabel tumor label
                for i in np.unique(multi_tumor_npy):
                    if i == 0 or i < 14 or i > 26:
                        continue
                    oar_tumor_npy[multi_tumor_npy == i] = i
                print("multi labeled tumor: ", np.unique(oar_tumor_npy))
            elif 14 in oar_tumor_pred_unique:
                ...
                multi_tumor_npy = oar_tumor_npy + 13  # oar[1-13] to tumor multi label[14-26]
                multi_tumor_npy[pseudo_tumor != 14] = 0  # stay tumor region
                oar_tumor_npy[pseudo_tumor == 14] = 27  # relabel tumor label
                for i in np.unique(multi_tumor_npy):
                    if i == 0 or i < 14 or i > 26:
                        continue
                    oar_tumor_npy[multi_tumor_npy == i] = i
                print("multi labeled merged tumor: ", np.unique(oar_tumor_npy))
            ...
            if 14 in gt_unique:
                ...
                for i in np.unique(multi_tumor_npy):
                    if i == 0 or i < 14 or i > 26:
                        continue
                    oar_tumor_npy[multi_tumor_npy == i] = i
                print("corrected multi labeled merged tumor: ", np.unique(oar_tumor_npy))
...
if __name__ == "__main__":
    ...
    task_name = "Task038_FLARE23noLabeledOARTumorMultiLabel"
    ...
    json_dict["labels"] = {
        ...
        "13": "Left_kidney",
        "14": "LiverTumor",
        "15": "Right_kidneyTumor",
        "16": "SpleenTumor",
        "17": "PancreasTumor",
        "18": "AortaTumor",
        "19": "Inferior_vena_cavaTumor",
        "20": "Right_adrenal_glandTumor",
        "21": "Left_adrenal_glandTumor",
        "22": "GallbladderTumor",
        "23": "EsophagusTumor",
        "24": "StomachTumor",
        "25": "DuodenumTumor",
